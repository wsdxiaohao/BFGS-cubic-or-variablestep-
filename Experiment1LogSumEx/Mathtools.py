...
def BFGSmetric(xkp,xk,gradkp,gradk,tGk,option=1):
    #input diagnal matrix lambdak*identity
    sk = xkp-xk
    yk = gradkp-gradk
    den = yk.T@sk
    vector = tGk@sk
    Gkp  = tGk +yk@yk.T/den-vector@vector.T/(vector.T@sk)    
    check = yk-tGk@sk
    test = np.sum(norm(check))#np.abs(den/((sk.T@sk)[0,0]))
    if test<1e-8:
        return tGk
    else:
        return Gkp
...
# To update the inverse of SR1 metric
def invBFGSmetric(xkp,xk,gradkp,gradk,invtGk):
    #input diagnal matrix lambdak*identity
    sk = xkp-xk
    yk = gradkp-gradk
    n = len(sk)
    P = (np.identity(n)-sk@yk.T/(yk.T@sk).item())
    Q = (np.identity(n)-yk@sk.T/(yk.T@sk).item())
    invGkp = P@invtGk@Q+ sk@sk.T/(yk.T@sk).item()
    return invGkp
