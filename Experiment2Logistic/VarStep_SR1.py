...
# SR1 regularized quasi Newton
def VarStep_SR1(Model, options, tol, maxiter=1000, check=10):
    ...
    for iter in range(1,maxiter+1):
        ...
        if S<kappa:
           ...
           dk = invGk@gradk
           norm_dk=norm(dk)[0]
           lambdak = 0.5*(((H/mu)*rkm-1) + np.sqrt(((H/mu)*rkm+1)**2+4*(H/mu)*norm_dk));
        ...
        invGkp =SR1metric(xkp,xk,gradkp,gradk,invhatGk)
